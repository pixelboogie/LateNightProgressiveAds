...
vid_tl
	...
	.from(vid_tt, 1.75, {scale:.9, transformOrigin:"15% 60%", ease:Elastic.easeOut}, 0)
...
end_tl
	...
	.from(end_tt, 1.75, {scale:.95, transformOrigin:"80% 25%", ease:Elastic.easeOut}, "t1")
	...
	.add("d1", "t1+=.3")
	// date
	.from(end_date, .6, {alpha:0, ease:Power1.easeOut}, "d1")
	
	// btn
	.from([end_cta1, end_cta2], .1, {alpha:0, ease:Back.easeOut}, "d1+=.1")
	.from([end_cta1, end_cta2], .4, {scale:.8, ease:Back.easeOut}, "d1+=.1")
	.call(btnReflect, [ctaBtnGrad[1], end_cta1Width], this, "d1+=.3") // [mc]

	// legal in
	//.from(end_legal, .5, {alpha:0, ease:Power0.easeOut}, "d1")
...
function dateChecker(){
	...
	if (currentDate >= targetDate[4] || urlDate === "4"){
		...
		// switch art
		vid_bg.src = "vid_bg2.jpg";
		end_bg.src = "end_bg2.jpg";
		...
		// adjust position
		TweenMax.set(vid_tt, {y:35});
		/*TweenMax.set(end_bg, {y:57});
		TweenMax.set(end_tt, {y:77});
		TweenMax.set(vid_replayBtn, {x:-265, y:77});
		TweenMax.set(end_txt, {y:77});
	...
	if (currentDate >= targetDate[2] || urlDate >= "2"){
		...
		// switch art
		vid_bg.src = "vid_bg.jpg";
		end_bg.src = "end_bg.jpg";
