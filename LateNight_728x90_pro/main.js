...
vid_tl
	...
	.from(vid_tt, 1.75, {scale:.9, transformOrigin:"35% 30%", ease:Elastic.easeOut}, 0)
...
end_tl
	...
	.from(end_bg, 2, {scale:1.1, transformOrigin:"5% 55%", ease:Power3.easeOut}, 0)
	...
	// btn
	.from(end_cta1, .1, {alpha:0, ease:Back.easeOut}, "d1+=.05")
	.from(end_cta1, .4, {scale:.8, ease:Back.easeOut}, "d1+=.05")
...
function btnReflect(btn, width) {
	...
	if (!TweenMax.isTweening(ctaBtnGrad)) {
		TweenMax.fromTo(ctaBtnGrad, .75, {x:-width+30}, {x:width-0, display:"inline", ease:Power0.easeOut});
